...
async def 설명(ctx):
    print('used help cmd')
    await ctx.respond("-도박장에 오신걸 환영합니다!:D-\n" + "\n"
...
async def 도박참여(ctx):
    if not userList:
        ...
        await ctx.respond(ctx.author.mention + "님이 게임에 참가하셨습니다.  (잔액:100000원)")
        return
        
    
    for name in userList:
        if name == str(ctx.author) :
            await ctx.respond(ctx.author.mention + "님은 이미 게임에 참가하셨습니다.")        
            return
...
async def 잔액(ctx):
    for name in userList:
        if name == str(ctx.author) :
            await ctx.respond(ctx.author.mention + "님의 잔액은 " + str(userList[name]) + "원 입니다.")
            return
...
async def 랭킹(ctx):
    ...
    if rankingMsg == "-랭킹-\n\n":
        await ctx.respond('참가자 없음.')    
        return

    await ctx.respond(rankingMsg)
